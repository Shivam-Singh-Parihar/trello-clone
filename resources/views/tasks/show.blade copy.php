                    ...
                    <div class="mb-4">
                        <div class="text-muted mb-2">
                            <small>
                                ...
                                <i class="fas fa-project-diagram me-1"></i> Project: <strong>{{ $task->project->name }}</strong>
                            </small>
                        </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if($task->assignee)
                                    <div class="d-flex align-items-center">
                                        <div class="avatar me-2" style="background-color: #{{ substr(md5($task->assignee->name), 0, 6) }}">
                            ...
                            <div class="mb-3">
                                ...
                                @if($task->due_date)
                                    <p class="{{ $task->due_date < now() ? 'text-danger' : '' }}">
                                        <i class="fas fa-calendar me-1"></i> {{ $task->due_date->format('F d, Y') }}
                                    </p>
                                @else
                                    ...
                                    <div class="attachment-item">
                                        ...
                                        <div class="attachment-details">
                                            ...
                                            <div class="attachment-info">
                                                <span>{{ number_format($attachment->size / 1024, 2) }} KB</span> &bull;
                                                <span>Uploaded by {{ $attachment->user->name }}</span> &bull;
                                                <span>{{ $attachment->created_at->diffForHumans() }}</span>
                                            </div>
                                        </div>
                                        <div class="attachment-actions">
                                            <a href="{{ route('attachments.download', $attachment) }}" class="btn btn-sm btn-outline-primary me-1">
                                ...
                                @foreach($comments as $comment)
                                    <div class="comment fade-in" id="comment-{{ $comment->id }}">
                                        <div class="comment-header">
                                            <div class="d-flex align-items-center">
                                                <div class="avatar me-2" style="background-color: #{{ substr(md5($comment->user->name), 0, 6) }}">
                                        ...
                                        @if($comment->user_id === auth()->id())
                                            <div class="mt-2 text-end">
                                                <button class="btn btn-sm btn-link text-muted edit-comment-btn" data-comment-id="{{ $comment->id }}" data-comment-body="{{ $comment->body }}">Edit</button>
