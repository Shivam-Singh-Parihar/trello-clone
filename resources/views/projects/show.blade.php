                    ...
                    <div class="kanban-column-header">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                <li>
                                    <a class="dropdown-item edit-list-btn" href="#" data-list-id="{{ $list->id }}"
                                    ...
                                    <div class="kanban-task-footer">
                                        <div class="task-badges">
                                            @if ($task->attachments->count() > 0)
                                                <span class="badge bg-light text-dark" data-bs-toggle="tooltip"
                                                    title="{{ $task->attachments->count() }} attachments">
                                                    <i class="fas fa-paperclip"></i> {{ $task->attachments->count() }}
                                                </span>
                                            @endif
                                            @if ($task->comments->count() > 0)
                                                <span class="badge bg-light text-dark" data-bs-toggle="tooltip"
                                                    title="{{ $task->comments->count() }} comments">
                                                    <i class="fas fa-comment"></i> {{ $task->comments->count() }}
                                                </span>
                                            @endif
                                            @if ($task->due_date)
                                                <span
                                                    class="badge {{ $task->due_date < now() ? 'bg-danger' : 'bg-light text-dark' }}"
                                        ...
                                        @if ($task->assignee)
                                            <div class="avatar avatar-sm"
                                                ...
                                                data-bs-toggle="tooltip" title="Assigned to {{ $task->assignee->name }}">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const drake = dragula(
                Array.from(document.querySelectorAll('.kanban-column-body')), {
                    moves: function(el, container, handle) {
                        return el.classList.contains('kanban-task') && !handle.classList.contains(
                            'edit-task-btn');
                    },
            ...
            // Handle task card click (view details)
            document.querySelectorAll('.kanban-task').forEach(task => {
                const viewLink = task.querySelector('.view-task-link');
                const editBtn = task.querySelector('.edit-task-btn');
                const taskId = task.getAttribute('data-task-id');

                // View task details when clicking anywhere on the card except the edit button
                task.addEventListener('click', function(e) {
                    // Don't trigger if clicking the edit button or a child of it
                    if (e.target.closest('.edit-task-btn')) {
                        return;
                    }
                    Livewire.dispatch('show-task-modal', {
                        taskId: taskId
                    });
                    e.preventDefault();
            ...
            document.querySelectorAll('.edit-task-btn').forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    const editTaskModal = new bootstrap.Modal(document.getElementById(
                        'editTaskModal'));
        ...
        `;
                    ...
                    fetch(`/tasks/${taskId}/edit`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                // Load the form HTML
                                editTaskContent.innerHTML = data.html;

                                // Initialize form with task data
                                const form = editTaskContent.querySelector('form');
                                if (form) {
                                    form.elements.title.value = data.task.title || '';
                                    form.elements.description.value = data.task.description ||
                                        '';

                                    if (data.task.due_date) {
                                        form.elements.due_date.value = data.task.due_date;
                                    }

                                    if (data.task.list_id) {
                                        form.elements.list_id.value = data.task.list_id;
                                    }

                                    if (data.task.assignee_id) {
                                        form.elements.assignee_id.value = data.task.assignee_id;
                                    }
                                }

                                // Initialize any plugins or datepickers
                    ...
                    <div class="alert alert-danger">
                        Error loading task form. Please try again.
                    </div>
                `;
                            }
                        })
